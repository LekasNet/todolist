    ...
    <div class="content">
      <div class="tasks">
        <div v-for="task in selectedFolderTasks" :key="task.id" class="task" :class="{ 'completed': task.completed }">
          ...
          <span v-if="editingTaskId !== task.id" @dblclick="() => startEditTask(task)">
      {{ task.text }}
    </span>
      ...
      <button @click="addingTask = true" class="add-task-button">Добавить запись</button>
...
.icon-button svg {
  width: 24px;
  height: 24px;
  fill: #333; /* Измените цвет, если используются SVG */
}
